...
const Index = () => {
  ...
  const fetchEthUsdData = useCallback(async () => {  
    ...
    try {
      const ethUsd220930: PremiumIndex = data.find((log: PremiumIndex) => log.symbol === "BTCUSD_220930")
      const ethUsd221230: PremiumIndex = data.find((log: PremiumIndex) => log.symbol === "BTCUSD_221230")
      const ethUsdPerp: PremiumIndex = data.find((log: PremiumIndex) => log.symbol === "BTCUSD_PERP")
      setEthUsdData({
  ...
  return <>
    <NextSeo
      titleTemplate="%s"
      title="Saya-chan ~btc-checker~"
      description="btc-checker"
      additionalLinkTags={[
        {
          rel: 'icon',
          sizes: '64x64',
          href: 'https://raw.githubusercontent.com/chan-san/saya-chan/main/public/BTC.png',
          type: 'image/png'
        }
      ]}
    />
    <VStack spacing="10">
      <Heading as='h1' mt="10">
        <Flex fontSize="28px" alignItems="center"><Image src={logo.src} marginRight=".3em" alt="BTC-logo" height="28px" />Saya-chan</Flex>
        <Text fontSize="14px" color="#888">~btc checker~</Text>
      </Heading>
      <VStack spacing="4">
        <Heading as='h2' fontSize="20px">Current mark price (Binance)</Heading>
        {(loading || !ethUsdData) ? <><Spinner />{errorMessage && <Text color="red">[Error]<br />{errorMessage}</Text>}</> : <EthUsdTable ethUsdData={ethUsdData} simulation={simulation} coin={'BTC'} setSimulation={setSimulationAndUpdate} />}
      </VStack>
      <VStack spacing="4" maxWidth="350px">
        <Heading as='h2' fontSize="20px">My position (entries)</Heading>
        <Entries ethUsdData={ethUsdData} coin={'BTC'} />
      </VStack>
    </VStack>
  </>
}
